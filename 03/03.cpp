	...
	const auto step = [&]()
	{
		...
		for(std::size_t y=0; y<height; ++y)
		{
			for(std::size_t x=0; x<width; ++x)
			{
				...
				ok&=y!=0 && x!=0 && (depth[(y-1)*width+(x-1)]==depth[y*width+x]);
				ok&=y!=height-1 && x!=width-1 && (depth[(y+1)*width+(x+1)]==depth[y*width+x]);
				ok&=y!=0 && x!=width-1 && (depth[(y-1)*width+(x+1)]==depth[y*width+x]);
				ok&=y!=height-1 && x!=0 && (depth[(y+1)*width+(x-1)]==depth[y*width+x]);
